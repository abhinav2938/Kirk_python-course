...
Value Filldown ROUTER_ID (\d{2,3}.\d{2,3}.\d{2,3}.\d{2,3})
Value NEIGHBOR (\S+)
Value REMOTE_AS (\d{4,5})
Value UP_DOWN (\w+)
Value STATE_PFXRCVD (\w+)

Start
  ^BGP.*identifier ${ROUTER_ID},.*number ${LOCAL_AS} 
  ^Neighbor.*PfxRcd.*$$ -> BGP_summ

BGP_summ
  ^${NEIGHBOR}\s+\d+\s+${REMOTE_AS}\s+.*\d\s+${UP_DOWN}\s+${STATE_PFXRCVD} -> Record
